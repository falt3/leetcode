...
private:
    ...
    bool dfs(int cource) {
        ...
        for (auto el : courses[cource]) {
            if (procesed[el] == 1) return false;
            if (procesed[el] == 2) continue;
...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        ...
        for (auto& el: prerequisites) {
            courses[el[0]].push_back(el[1]);
        }
        procesed.resize(numCourses, 0);
...
int main() {
    ...
    auto res = Solution{}.canFinish(100, v2);
    cout << "res:" << res << endl;
